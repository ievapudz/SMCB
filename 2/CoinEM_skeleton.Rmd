...
coin_EM <- function(D, k, lambda, theta, tolerance = 1e-2) {
  ...
  while (diff > tolerance) {
    ...
    for (i in 1:ncol(gamma)) {
        ...
        P_obs_i_given_CA <- lambda_all[1]*(theta_all[1]^nheads)*((1-theta_all[1])^ntails)
        P_obs_i_given_CB <- lambda_all[2]*(theta_all[2]^nheads)*((1-theta_all[2])^ntails)

        # Saving likelihoods for the next step
        c_P_obs_i_given_CA <- c(c_P_obs_i_given_CA, P_obs_i_given_CA)
        c_P_obs_i_given_CB <- c(c_P_obs_i_given_CB, P_obs_i_given_CB)
    ...
    ll_hid_new <- 0
    for (i in 1:ncol(gamma)) {
        ll_hid_new <- ll_hid_new+gamma[1,i]*log(lambda_all[1])+gamma[1,i]*log(c_P_obs_i_given_CA[i])
        ll_hid_new <- ll_hid_new+gamma[2,i]*log(lambda_all[2])+gamma[2,i]*log(c_P_obs_i_given_CB[i])
    }
