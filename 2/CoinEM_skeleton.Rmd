...
# Create a data frame with iterations and ll_obs
data <- data.frame(iterations = 1:(length(res$ll_obs)-1), ll_obs = res$ll_obs[2:length(res$ll_obs)])
...
Probability of heads through iterations (blue - coin A; red - coin B)

```{r}
#print(res$theta_all)
# Create a data frame with iterations and theta values
theta_data <- data.frame(iterations = 1:(length(res$theta_all[,1])), theta_CA = res$theta_all[,1], theta_CB = res$theta_all[,2])
...
ggplot(theta_data, aes(x = iterations)) +
  ...
  labs(x = "Iterations", y = "Probability of Heads") +
  theme_minimal() +
  theme(text = element_text(size = 20))
...
```{r, eval=FALSE}
library(rmarkdown)
render("CoinEm_skeleton.Rmd", pdf_document(TRUE), "2.pdf")
```
