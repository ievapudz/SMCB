...
```{r}
print(res$ll_obs)
print(length(res$ll_obs)-1)
#ggplot(1:(length(res$ll_obs)-1), res$ll_obs[2:length(res$ll_obs)])
library(ggplot2)
...
ggplot(data, aes(x = iterations, y = ll_obs)) +
  ...
  theme_minimal() +
  coord_cartesian(xlim = c(2, length(res$ll_obs)), ylim = c(0, max(res$ll_obs))) +  # Set aspect ratio to 1 for a square plot
  theme(text = element_text(size = 20))  # Increase font size to 14
