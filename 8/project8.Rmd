...
edges_model1 <- c('S1', 'S3', 
                  ...
                  "S4", "E3",
                  "S4", "S5",
                  "S5", "E5", #from here transitive closure 
                  "S1", "S5"
                  )
...
edges_model2 <- c("S1", "S4", "S1", "E1", "S1", "E2", "S2", "E4","S2", "E6","S2", "S5",
                  "S3", "S1", "S3", "S5", "S3", "S4", "S4", "E3", "S4", "S5", "S5", "E5", #from here transitive closure 
                  "S1", "S5")
...
network_score_1 = scoreAdj(D1, adj=phi_1, method="disc", fpfn=c(0.05,0.01), logtype=exp(1))$score
network_score_2 = scoreAdj(D2, adj=phi_2, method="disc", fpfn=c(0.05,0.01), logtype=exp(1))$score
...
1.Using the definitions for HM-NEMs from the lecture, compute the transition 
probabilities from $G_t = u$ to $G_{t+1} \in {v1, v2}$ for different smoothness 
parameter $\lambda \in {0.1, . . . , 0.9}$.
...
print(T)
```
2. Plot the transition probabilities for $v_{1}$ and $v_{2}$ as a function of $\lambda$. Describe the trans
...
2.Assume that {C1,C2} are generated from F1 and {C3,C4} are generated from F2, compute the noiseless log odds matrix R, where R_{jc} > 0 means that the perturbation on cell c has an effect on E-gene j:
...
```{r, eval=FALSE}
library(rmarkdown)
render("project8.Rmd", pdf_document(TRUE), "Indilewitsch_Toidze_Houhamdi_Pudziuvelyte_Project8.pdf")
#rmarkdown::render("project8.Rmd", output_format = "pdf_document")
