...
main <- function() {
    aln_ATP <- parseAlignment("data/ATPases.txt")
    aln_GTP <- parseAlignment("data/GTP_binding_proteins.txt")

    profile_ATP <- learnHMM(aln_ATP)
    profile_GTP <- learnHMM(aln_GTP)

    # TODO: identify positions with the highest insertion ($iE) and match ($mE) emission frequencies
    #print(profile_ATP$iE)
    print(names(profile_ATP))
    #print(profile_ATP$T)
}
