...
```{r}
GTP_alignment <- parseAlignment("data/GTP_binding_proteins.txt")
ATP_alignment <- parseAlignment("data/ATPases.txt")
...
ggplot(df_emi_GTP, aes(x = rownames, y = freq))+
  ...
  labs(x="amino acids", y = "frequency", title = paste("Match emission frequency",
...
ggplot(df_ins_GTP, aes(x = rownames, y = freq))+
  geom_bar(stat = "identity")+
  labs(x="amino acids", y = "frequency", title = paste("Insert emission frequency",
...
```{r}
unclass_proteins <- parseProteins("data/Unclassified_proteins.txt")
```
...
```{r, eval=FALSE}
library(rmarkdown)
render("project4.Rmd", pdf_document(TRUE), "Project4_sol.pdf")
```
