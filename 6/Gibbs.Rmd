...
After around 1000 iterations, the fluctuations around the potential scale reduction factor are negligible, therefore a suggested burn-in time is 1000.

## Re-estimation of marginal probability

Computing marginal probability P(R=T|S=T,W=T) excluding the samples from the burn-in phase.

```{r}
burn_in_phase <- 1000
result1 <- table(samples1[(burn_in_phase+1):N, "R"], samples1[(burn_in_phase+1):N, "C"])/N
rownames(result1) <- c("R0", "R1")
colnames(result1) <- c("C0", "C1")

result2 <- table(samples2[(burn_in_phase+1):N, "R"], samples2[(burn_in_phase+1):N, "C"])/N
rownames(result2) <- c("R0", "R1")
colnames(result2) <- c("C0", "C1")

marginal1.R.S1_W1 <- sum(result1[2,])
marginal2.R.S1_W1 <- sum(result2[2,])
print(paste("Marginal probability using the first run of 50000 iterations and excluded burn-in phase:", marginal1.R.S1_W1))
print(paste("Marginal probability using the second run of 50000 iterations and excluded burn-in phase:", marginal2.R.S1_W1))
```

## Analytical solution of marginal probability
